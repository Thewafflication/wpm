...
static int load_installed(installed_entry* result,int* count){char root[PATH_SIZE],store[PATH_SIZE],search[PATH_SIZE],path[PATH_SIZE];WIN32_FIND_DATAA item;HANDLE find;*count=0;if(!wpm_get_data_root(root,sizeof(root))||!join_path(store,sizeof(store),root,"packages")||!join_path(search,sizeof(search),store,"*.zip"))return 0;find=FindFirstFileA(search,&item);if(find==INVALID_HANDLE_VALUE)return GetLastError()==ERROR_FILE_NOT_FOUND;do{wpm_package_info info;semver parsed;if(*count>=MAX_INSTALLED||!join_path(path,sizeof(path),store,item.cFileName))continue;if(!wpm_archive_inspect(path,&info)||!semver_parse(info.version,&parsed)){printf("Warning: invalid installed package record: %s\n",item.cFileName);continue;}installed_entry*e=&result[(*count)++];strcpy_s(e->name,sizeof(e->name),info.name);strcpy_s(e->version,sizeof(e->version),info.version);strcpy_s(e->arch,sizeof(e->arch),info.arch);}while(FindNextFileA(find,&item));FindClose(find);return 1;}
