...
static package_entry* select_candidate(package_entry* entries,int count,const char* name,const char* arch,const char* exact_version,int install_mode){package_entry*selected=NULL;int allow_pre=prerelease_effective(name,NULL),named=0,version_matched=0,arch_matched=0;for(int i=0;i<count;i++){semver parsed;if(_stricmp(entries[i].name,name)!=0)continue;named++;if(!semver_parse(entries[i].version,&parsed)){printf("Warning: ignoring invalid SemVer %s for %s.\n",entries[i].version,name);continue;}if(exact_version&&strcmp(entries[i].version,exact_version)!=0)continue;if(is_prerelease(entries[i].version)&&!allow_pre)continue;version_matched++;if(install_mode&&!arch){if(_stricmp(entries[i].arch,WPM_TARGET_ARCH)!=0&&_stricmp(entries[i].arch,"any")!=0)continue;if(selected&&_stricmp(selected->arch,WPM_TARGET_ARCH)==0&&_stricmp(entries[i].arch,"any")==0)continue;if(selected&&_stricmp(selected->arch,"any")==0&&_stricmp(entries[i].arch,WPM_TARGET_ARCH)==0){selected=&entries[i];arch_matched++;continue;}}else if(_stricmp(entries[i].arch,arch)!=0)continue;arch_matched++;if(better_candidate(&entries[i],selected))selected=&entries[i];}REPO_VERBOSE("resolution for '%s': name matches=%d, version/prerelease matches=%d, architecture matches=%d, selected=%s",name,named,version_matched,arch_matched,selected?selected->version:"none");return selected;}
