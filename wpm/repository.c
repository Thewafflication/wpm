...
int wpm_config_prerelease_set(const char* package_name,int enabled){const char*scope=package_name?package_name:"global";if(!rewrite_prerelease(package_name,enabled,0)){printf("Error: could not save prerelease configuration.\n");return 0;}printf("Prerelease %s=%s.\n",scope,enabled?"true":"false");printf("Result: prerelease configured %s %s\n",scope,enabled?"true":"false");return 1;}
